...
@endsection
<div>
    @include('livewire.modal.daftar-vendor-modal')
    <!-- Page Heading -->
                   ...
                   <div class="table-responsive">
                                ...
                                @else
                                    @foreach ($vendor as $v)
                                        <tr>
                                            ...
                                            <td scope="row">{{ Carbon\Carbon::parse($v->created_at)->translatedFormat('d/m/Y')}}</td>
                                            <td class="text-center">
                                                <button wire:click='ubah({{$v->id}})' class="btn btn-primary btn-circle btn-sm" data-toggle="tooltip" data-placement="bottom" title="Ubah">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button wire:click='hapus({{$v->id}})' class="btn btn-danger btn-circle btn-sm" data-toggle="tooltip" data-placement="bottom" title="Hapus">
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <span>Halaman : {{ $vendor->currentPage() }} </span><br/>
                            <span>Jumlah Data : @if($search == '') {{$total}} @else {{$vendor->count() }} @endif  </span><br/>
                            <span>Data Per Halaman : {{ $vendor->perPage()}} </span><br/><br/>
                        </div>
