...
@endsection
<div>
    @include('livewire.modal.daftar-instansi-modal')
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Daftar Instansi</h1>
    </div>
                   ...
                   <div class="table-responsive">
                                ...
                                @else
                                    @foreach ($instansi as $i)
                                        <tr>
                                            ...
                                            <td scope="row">{{ Carbon\Carbon::parse($i->created_at)->translatedFormat('d/m/Y')}}</td>
                                            <td class="text-center">
                                                <button wire:click='ubah({{$i->id}})' class="btn btn-primary btn-circle btn-sm" data-toggle="tooltip" data-placement="bottom" title="Ubah">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button wire:click='hapus({{$i->id}})' class="btn btn-danger btn-circle btn-sm" data-toggle="tooltip" data-placement="bottom" title="Hapus">
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <span>Halaman : {{ $instansi->currentPage() }} </span><br/>
                            <span>Jumlah Data : @if($search == '') {{$total}} @else {{$instansi->count() }} @endif  </span><br/>
                            <span>Data Per Halaman : {{ $instansi->perPage()}} </span><br/><br/>
                        </div>
